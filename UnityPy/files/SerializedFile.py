...
class SerializedFile:
	...
	def __init__(self, assets_manager, full_name: str, reader: EndianBinaryReader):
		...
		if header.version >= 8:
			...
			try:
				self._target_platform = BuildTarget(self._m_target_platform)
			except KeyError:
				self._target_platform = BuildTarget.UnknownPlatform
	...
	def save(self) -> bytes:
		...
		if header.version >= 9:
			...
			NotImplementedError("old header")
		...
		for typ in types:
			...
			if header.version >= 13:
				...
				writer.write_bytes(typ._old_type_hash)  # Hash128

			if self._enable_type_tree:
				if header.version >= 12 or header.version == 10:
					raise NotImplementedError("type tree 5")
				# write type tree 5
				else:
